...
#   #   # #####################################################################
  ...
  if (args['dynamic']) {
    ...
    writeCodeToFile(
      () => generateIconMetaMap(metadata),
      'lib/meta_icon_mapping.dart',
    );
  } else {
...
/// Creates a map of search strings and full FontAwesome meta objects
///
/// To use, import:
/// `import 'package:font_awesome_flutter/meta_icon_mapping.dart'`
/// and then use `getIconsMetaListFromSearch(String match)` to query for icons.
List<String> generateIconMetaMap(List<IconMetadata> icons) {
  print(blue('Generating FontAwesomeIconMetadata and search terms mapping'));
  ...
  List<String> output = [
    ...
    '/// that is created by searching a map string of label and search terms',
    '///',
    '/// Returns an empty list if no icons label or search terms match.',
  ...
  for (var icon in icons) {
    List<String> listTerms = icon.searchTerms;
    String iconTerms = "";
    String searchString = icon.name.replaceAll("'", "");
    for (String term in listTerms) {
      if (iconTerms.isNotEmpty) iconTerms += ',';
      iconTerms += "r'${term.replaceAll("'", "")}'";
      searchString += ' ' + term.replaceAll("'", "");
    }
    String iconNames = "";
    String iconStyles = "";
    for (String style in icon.styles) {
      if (iconNames.isNotEmpty) iconNames += ',';
      if (iconStyles.isNotEmpty) iconStyles += ',';
      iconNames += 'FontAwesomeIcons.' +
          normalizeIconName(icon.name, style, icon.styles.length);
      iconStyles += "r'$style'";
    }

    output.add(
        "r'${searchString.toLowerCase()}' : FontAwesomeIconMetadata(r'${icon.name.replaceAll("'", "")}', r'${icon.label.replaceAll("'", "")}', r'${icon.unicode.replaceAll("'", "")}', [ $iconNames ], [ $iconTerms ], [ $iconStyles ]) ,");
  }
